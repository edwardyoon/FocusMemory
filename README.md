...
fewer than 2 entries, or (c) any entry's content contains a literal `<da:` (which would corrupt the
server's numbering check) — in that case the server would fail open anyway, so it is cleaner to skip
the injection.
