...
dotenv.config({ override: true, quiet: true, path: path.join(path.dirname(fileURLToPath(import.meta.url)), "..", ".env") });
 ...
 */
export async function pruneAndSummarize(query, results, { useLLM = true } = {}) {
  if (!results || results.length === 0) return null;

  // P5-opt: Skip LLM for small result sets — keyword extract is faster and sufficient.
  // useLLM=false: the UserPromptSubmit hook has an 8s budget — the LLM summary
  // (10~30s on the shared 123:8080 qwen27b) would blow it and drop the injection.
  if (results.length <= 4 || !useLLM) {
...
- **Source attribution is mandatory**: after each distinct fact, append the source file of that fact in exactly this format: \`[출처 : file_path]\` (use only file paths that appear in the fragments). Example: "nginx가 upstream으로 8개 컨테이너에 트래픽을 분산한다. [출처 : docs/deploy-architecture.md]"
- If one fact comes from multiple files, list them all in a single tag: \`[출처 : file_a, file_b]\`.
...
- If search results have no answer, output "There is not enough information on this topic."`;
  ...
  try {
    ...
    const res = await fetch(SUMMARY_LLM_URL, {
      ...
      body: JSON.stringify({
        model: SUMMARY_LLM_MODEL,
        // chat-completions contract: the server (llama.cpp) separates thinking
        // into choices[0].message.reasoning_content, keeping .content clean
    ...
    return appendSourceHint(text.trim(), results);
  } catch (err) {
    if (err.name === "AbortError") {
...
/**
 * Resolve the source file path for a result payload, if any backend recorded one.
 * Covers docs (source_doc), graph nodes (file), graph edges (source_file),
 * work_memory / code_structure (related_files[0]).
 */
function sourceFileOf(payload) {
  if (!payload) return "";
  return payload.source_doc || payload.file || payload.source_file
    || (payload.related_files && payload.related_files[0]) || "";
}

/**
 * Append the read_file hint line when at least one result carries a source file,
 * so the consuming agent knows it can open the [출처 : file] tag directly.
 */
function appendSourceHint(text, results) {
  if (!text) return text;
  const hasSource = (results || []).some(r => sourceFileOf(r.payload));
  return hasSource ? `${text}\n\n(세부 확인이 필요할 때 위 [출처 : file] 파일을 read_file 도구로 직접 읽어 확인하세요)` : text;
}
...
function lightweightKeywordSummary(query, results) {
  ...
  for (const [i, s] of top.entries()) {
    ...
    const src = sourceFileOf(p);
    summary += `${i + 1}. [${col}] ${title}${src ? ` [출처 : ${src}]` : ""}\n`;
    if (p.detail) {
  ...
  return appendSourceHint(summary.trim(), top.map(s => s.result));
}

// ─── Triviality gate (shared by MCP search_memory + HTTP auto-recall hook) ──

/**
 * Rule-based (no LLM) heuristic: is this query cheap enough that a backend
 * lookup is not worth it? Ported from the /v1/context/search HTTP hook so both
 * entry points skip with identical criteria. A skipped query still satisfies
 * the Hard Gate (the tool call / hook ran) — it just pays no backend cost.
 *
 * @param {string} query Raw user query
 * @returns {boolean} true when the query should be skipped without backend access
 */
export function isTrivialQuery(query) {
  const q = (query || "").trim();
  if (q.length < 10) return true;
  // Pure math / numeric strings: "2+2", "(1+2)*3"
  if (/^[\d+\-*/().\s=]+$/.test(q)) return true;
  // Greetings / single acknowledgements: "hi", "ok!", "thanks."
  if (/^(hi|hello|hey|yo|ok|thanks|done|yes|no)\s*[!.]?\s*$/i.test(q)) return true;
  return false;
}
 ...
 * - project_facts  : Meilisearch text search
 */
const BACKENDS = ["work_memory", "graph", "decision_chains", "project_facts"];
...
export function routeQuery(query, features) {
  ...
  for (const b of BACKENDS) {
    ...
    if (b === "work_memory") {
      ...
      // Docs / architecture / policy: prioritize for knowledge queries and low identifier ratio
      if (features.is_knowledge) spec += 0.9;
      if (features.identifier_ratio < 0.1) spec += 0.5;
      if (!features.is_causal && !features.is_temporal && !features.is_structural) spec += 0.3;
      // Penalize code-level or causal queries
      if (features.identifier_ratio > 0.3) spec -= 0.4;
      if (features.is_causal) spec -= 0.2;
    }
...
// ─── Merged-result reranking (shared by MCP search_memory, HTTP hook, taskReceiver) ──
 ...
 *
 * Backend weights are intent-aware:
 * - Decision queries (causal/temporal, non-knowledge): decision records
 *   (decision_chains, work_memory) are prioritized over project_facts — a
 *   "what did we decide / why" query must surface decisions, not planning docs.
 * - All other queries: project_facts keeps top priority (docs are the
 *   authoritative source for general context).
 *
 * @param {Array<{score?: number, payload: object, _collection?: string}>} allResults
 * @param {{is_causal?: boolean, is_temporal?: boolean, is_knowledge?: boolean} | null} [features]
 *   Query features from extractQueryFeatures(). Omit (or pass null) to use the
 *   default (non-decision) weights.
 * @returns {Array} results sorted by rerank_score (descending)
 */
export function rerankMerged(allResults, features = null) {
  const now = Date.now();
  const DAY_MS = 86400000;

  const isDecisionQuery = Boolean((features?.is_causal || features?.is_temporal) && !features?.is_knowledge);
  const backendWeights = isDecisionQuery
    ? { decision_chains: 1.3, work_memory: 1.2, project_facts: 1.0, graph: 1.0, code_structure: 0.9 }
    : { project_facts: 1.3, decision_chains: 1.1, work_memory: 1.0, graph: 1.0, code_structure: 0.9 };
