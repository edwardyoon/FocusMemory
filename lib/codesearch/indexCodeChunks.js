...
/**
 * Delete code_chunks points whose file no longer exists on disk.
 * Mirrors the deleted-file handling in buildGraph.js (Phase B) — without it,
 * chunks of removed files keep surfacing in semantic search forever.
 * @param {Set<string>} currentRelPaths - relPaths of files present on disk
 * @returns {Promise<number>} number of orphan points deleted
 */
async function cleanOrphanChunks(currentRelPaths) {
  const indexed = new Map(); // file_path -> point ids
  let offset = null;
  do {
    const req = { limit: 1000, with_payload: ["file_path"] };
    if (offset) req.offset = offset;
    const res = await qdrant.scroll("code_chunks", req);
    for (const p of res.points) {
      const fp = p.payload && p.payload.file_path;
      if (typeof fp !== "string") continue;
      if (!indexed.has(fp)) indexed.set(fp, []);
      indexed.get(fp).push(p.id);
    }
  ...
  const orphanIds = [];
  const orphanFiles = [];
  for (const [fp, ids] of indexed) {
    if (!currentRelPaths.has(fp)) {
      orphanFiles.push(fp);
      orphanIds.push(...ids);
    }
  }
  if (orphanIds.length === 0) {
    console.log("  (no orphan chunks)");
    return 0;
  }
  for (let i = 0; i < orphanIds.length; i += 500) {
    await qdrant.delete("code_chunks", { wait: true, points: orphanIds.slice(i, i + 500) });
  }
  for (const fp of orphanFiles.slice(0, 20)) {
    console.log(`  [orphan] ${fp} (${indexed.get(fp).length} chunks)`);
  }
  if (orphanFiles.length > 20) console.log(`  ... and ${orphanFiles.length - 20} more files`);
  console.log(`  [orphan] removed ${orphanIds.length} chunks from ${orphanFiles.length} deleted file(s)`);
...
async function main() {
  ...
  // Orphan cleanup — full-scan mode only. With a targetDir subset the scan
  // set is incomplete, so diffing against it would delete chunks that belong
  // to files outside the subset. --force already cleared the collection.
  let orphanRemoved = 0;
  if (!force && !targetDir) {
    console.log("--- Orphan cleanup (chunks of deleted files) ---");
    try {
      const currentRelPaths = new Set(files.map((f) => path.relative(scanRoot, f)));
      orphanRemoved = await cleanOrphanChunks(currentRelPaths);
    } catch (err) {
      console.error("  ✗ orphan cleanup failed: %s", err.message);
    }
    console.log("");
  }
  ...
  console.log("  Orphan chunks removed: %d", orphanRemoved);
  console.log("=== Done ===");
}
